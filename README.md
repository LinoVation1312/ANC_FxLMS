...
Checklist
- [ ] Pick repo name: LinoVation1312/ANC_FxLMS
- [ ] Create a minimal .gitignore (Python, macOS, large artifacts)
- [ ] Commit and push (CLI or manual)
...
git branch -m main
git add .
git commit -m "Initial commit: FxLMS ANC"
```
...
gh auth login
# Create and push
gh repo create LinoVation1312/ANC_FxLMS --public --source=. --remote=origin --push
```

Option B — Manual (HTTPS)
```bash
# Create an empty repo on GitHub: https://github.com/LinoVation1312/ANC_FxLMS (no README)
git remote add origin https://github.com/LinoVation1312/ANC_FxLMS.git
...
```bash
printf "%s\n" \
"venv/" "__pycache__/" "*.pyc" ".DS_Store" ".env" \
"*.wav" "*.mat" \
"mic_sensitivity.json" \
> .gitignore
```

Notes
- Use SSH instead of HTTPS if preferred (setup ssh-agent and add your key on GitHub).
- For large recordings, consider Git LFS.